...
class Cell:
    ...
    def prolife(self, field, celllist):
        ni = self.i + Cell.mi
        nj = self.j + Cell.mj
        cell_new = Cell(ni, nj)
    ...
    def move(self, field, celllist):
        ...
        if field[self.i, self.j] >= 0:
            ...
            celllist[id].i = ni
            celllist[id].j = nj
            celllist[id].move(field, celllist)
            field[self.i, self.j] = self.id
        else:
            field[self.i, self.j] = self.id

    @classmethod
    def radial_prolife_up(cls, field, on, func, celllist):
        if celllist[field[on, on]].proliferation == 1:
            getattr(celllist[field[on, on]], func)(field)
            celllist[field[on, on]].prolife(field)
        for r in range(1, on):
