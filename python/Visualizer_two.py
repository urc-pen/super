...
class Visualizer_two(Visualizer):
    ...
    def append_cell_num(self, heatmap, t):
        self.onelist.append(np.sum(heatmap == 1) + 1)
        self.twolist.append(np.sum(heatmap == 2) + 1)
        self.tlist.append(t)
    ...
    def first_heatmap_graph(self, heatmap):
        ...
        self.ax1.plot(self.tlist, self.onelist, label=self.label1, color=self.colors[1])
        self.ax1.plot(self.tlist, self.twolist, label=self.label2, color=self.colors[2])
    ...
    def plot_append_heatmap_graph(self, heatmap, t, plot=False, append=True):
            if plot == True or append == True:
                for n in range(0, 3):
                    heatmap[0, n] = n
                im = self.ax1.plot(self.tlist, self.onelist, label=self.label1, color=self.colors[1])
                im += self.ax1.plot(self.tlist, self.twolist, label=self.label2, color=self.colors[2])
