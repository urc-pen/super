...
class Visualizer:
    ...
    def first_heatmap_graph(self, heatmap, tlist, threelist, fourlist, fivelist, sixlist):
        ...
        self.ax1.plot(tlist, threelist, label="3", color=self.colors[3])
        self.ax1.plot(tlist, fourlist, label="4", color=self.colors[4])
        self.ax1.plot(tlist, fivelist, label="5", color=self.colors[5])
        self.ax1.plot(tlist, sixlist, label="6", color=self.colors[6])
    ...
    def plot_append_heatmap_graph(self, heatmap, t, tlist, threelist, fourlist, fivelist, sixlist, plot=False, append=True):
            if plot == True or append == True:
                for n in range(0, 7):
                    heatmap[0, n] = n
                im = self.ax1.plot(tlist, threelist, label="3", color=self.colors[3])
                im += self.ax1.plot(tlist, fourlist, label="4", color=self.colors[4])
                im += self.ax1.plot(tlist, fivelist, label="5", color=self.colors[5])
                im += self.ax1.plot(tlist, sixlist, label="6", color=self.colors[6])
    ...
    def save_heatmap_graph(self, mode, para, heatmap, tlist, threelist, fourlist, fivelist, sixlist):
        if mode == "pic":
            self.ax1.plot(tlist, threelist, label="3", color=self.colors[3])
            self.ax1.plot(tlist, fourlist, label="4", color=self.colors[4])
            self.ax1.plot(tlist, fivelist, label="5", color=self.colors[5])
